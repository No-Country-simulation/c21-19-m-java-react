...
const RegistrosCRUD = () => {
    ...
    const eliminarRegistro = async (admin, mascota) => {
		try {
			...
			setRegistros(
				registros.filter((registro) => registro.id_mascotas !== mascota || registro.alias !== admin)
			);
		} catch (error) {
			console.error(error);
		}
                ...
                <tbody>
                    {registros.map((registro, index)=>(
                        <tr key={index}>
                            ...
                            <td>
                                <button onClick={() => eliminarRegistro(registro.alias,registro.id_mascotas)}>Eliminar</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
